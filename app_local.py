...
# app_local.py
# Render版 app_v10_61 の見た目をベースに、保存先だけ local_store/local_data に差し替えたローカル運用版。
...
def display_player_evidence_label(tone, item):
    """
    既存の材料タグを、買い判断で見やすい言葉に置き換える。
    DB/CSV/保存処理は触らず、画面表示だけを変える。
    """
    ...
    key = item.replace(" ", "").replace("モータ", "モーター")

    plus_map = {
        "勝率系": "地力上位", "勝率": "地力上位", "地力": "地力上位",
        "当地": "当地得意", "地元": "地元水面",
        "モーター": "機力良さげ", "機力": "機力良さげ", "ボート": "ボート良",
        "ST": "ST良さげ", "スタート": "ST良さげ",
        "コース": "コース得意", "近況": "近況良い", "級別": "級別上位",
        "B2補正": "B2でも実力あり", "別格": "実力別格", "実力A1級": "実力A1級",
        "展示": "展示良い", "進入": "進入取り良い", "ピット": "ピット良さげ",
    }
    minus_map = {
        "勝率系": "地力不安", "勝率": "地力不安", "地力": "地力不安",
        "当地": "当地不安", "地元": "地元利なし",
        "モーター": "機力不安", "機力": "機力不安", "ボート": "ボート不安",
        "ST": "ST不安", "スタート": "ST不安",
        "コース": "コース不安", "近況": "近況不安", "級別": "級別不安",
        "展示": "展示不安", "展示F": "展示F注意", "進入": "進入変化注意", "ピット": "ピット不安",
    }
    neutral_map = {
        "勝率系": "地力確認", "勝率": "地力確認", "当地": "当地確認", "地元": "地元水面",
        "モーター": "機力確認", "機力": "機力確認", "ST": "ST確認", "スタート": "ST確認",
        "コース": "コース確認", "近況": "近況確認", "級別": "級別確認",
        "展示": "展示確認", "展示F": "展示F注意", "進入": "進入変化",
        "チルト": "チルト確認", "チルト高": "伸び注意", "部品交換": "整備変更",
        "安定板": "安定板あり", "ピット": "ピット気配",
    }

    if tone == "plus":
        label = plus_map.get(key) or neutral_map.get(key) or item
        role = "plus"
    elif tone == "minus":
        label = minus_map.get(key) or neutral_map.get(key) or item
        role = "minus"
    else:
        label = neutral_map.get(key) or item
        role = "neutral"

    if any(w in label for w in ["注意", "確認", "変化", "整備", "チルト", "安定板"]):
        if role == "plus" and label not in {"地力上位", "当地得意", "地元水面", "機力良さげ", "ST良さげ", "展示良い"}:
            role = "notice"
        elif role == "neutral":
            role = "notice"
...
def render_player_evidence_chips(reason_items, default_items):
    ...
    for tone, item in list(reason_items) + list(default_items):
        ...
        label, role = display_player_evidence_label(tone, item)
        if not label:
            continue
        ...
        if role == "plus":
            ...
            cls = "player-evidence-chip player-evidence-chip-notice"
            mark = "!"
        else:
            cls = "player-evidence-chip player-evidence-chip-neutral"
            mark = "・"

        chips.append(f'<span class="{cls}"><span class="player-evidence-mark">{mark}</span>{label}</span>')
    ...
    <div class="app-shell">
      <div class="topbar">
        <div class="brand">
          ...
          <div>
            <div class="brand-title">Race Candidates Local</div>
      ...
      <div class="header hero hero-strong">
        ...
        <div class="nav nav-app">
          ...
          <form method="post" action="/run_latest_local" class="nav-form" onsubmit="return confirm('collectorで直前更新を実行します。よろしいですか？');">
            <button type="submit" class="nav-card nav-card-update nav-button">直前更新</button>
          </form>
...
def render_history_page(date_summaries):
    ...
    return render_layout("過去データ", f'<div class="app-shell"><div class="topbar"><div class="brand"><div class="brand-logo">🗂️</div><div><div class="brand-title">Race Candidates Local</div><div class="brand-sub">過去データ一覧</div></div></div></div><div class="header hero hero-strong"><div class="title">過去データ</div><div class="nav nav-app"><a href="/" class="nav-card">今日の候補</a><a href="/stats" class="nav-card">今日の集計</a><a href="/history" class="nav-card active">過去データ</a></div></div>{list_html}</div>')
...
def render_history_detail_page(
    ...
    content = f'''
    <div class="app-shell">
      <div class="topbar"><div class="brand"><div class="brand-logo">🧾</div><div><div class="brand-title">Race Candidates Local</div><div class="brand-sub">過去データ詳細</div></div></div><div class="topbar-status"><span class="top-pill">対象日: {race_date}</span></div></div>
...
def render_layout(title, body_html):
    ...
    <style>
      *{box-sizing:border-box}
      ...
      .nav-button{border:none;cursor:pointer;font:inherit}
      .nav-card-update{background:#101828;color:#fff}
      ...
      .player-evidence-chip{display:inline-flex;align-items:center;gap:4px;padding:5px 9px;border-radius:999px;border:1px solid;font-size:12px;font-weight:800;line-height:1.2;white-space:nowrap}
      .player-evidence-mark{font-size:11px;font-weight:900;line-height:1}
      ...
      .player-evidence-chip-notice{background:#fff6e5;border-color:#f5deb3;color:#b54708}
...
# ===== local_store adapter: Render版UIをそのまま使い、DBだけ local_data に差し替え =====
def db_connect():
    raise RuntimeError("local版ではDATABASE_URL/PostgreSQLは使いません")
...
def is_valid_import_token(req):
    if not IMPORT_TOKEN:
        return True
    return str(req.headers.get("X-IMPORT-TOKEN", "")).strip() == IMPORT_TOKEN
...
def _local_row_key(row):
    return local_store.race_key(row)


def _local_id_for_key(key):
    return int(zlib.crc32(str(key).encode("utf-8")) & 0x7fffffff)


def _local_defaults():
    cols = [c.strip() for c in CARD_SELECT_COLUMNS.replace("\n", " ").split(",") if c.strip()]
    d = {c: "" for c in cols}
    d.update({
        "id": 0,
        ...
        "payout": 0,
        "memo": "",
        "result_trifecta_payout": 0,
        "result_exacta_payout": 0,
        "result_trio_payout": 0,
        "settled_flag": 0,
    })
    return d
...
def _normalize_row_for_render(row, purchases=None):
    base = _local_defaults()
    base.update(dict(row or {}))
    base["race_date"] = str(base.get("race_date") or today_text()).strip()
    base["race_no"] = local_store.normalize_race_no(base.get("race_no") or base.get("race_no_num"))
    base["race_no_num"] = local_store.normalize_race_no_num(base.get("race_no") or base.get("race_no_num"))
    base["candidate_source"] = normalize_candidate_source(base.get("candidate_source"))
    key = _local_row_key(base)
    base["_local_key"] = key
    base["id"] = _local_id_for_key(key)

    # local_store側のJSONでは list で持つ場合があるので、Render版のparse_json_array_textに合わせてJSON文字列へ寄せる。
    for json_key in ["ai_reasons", "exhibition"]:
        value = base.get(json_key)
        if isinstance(value, (list, dict)):
            base[json_key] = json.dumps(value, ensure_ascii=False)
        elif value is None or str(value).strip() == "":
            base[json_key] = "[]"

    for int_key in ["amount", "race_no_num", "purchased", "hit", "payout", "result_trifecta_payout", "result_exacta_payout", "result_trio_payout", "settled_flag"]:
        base[int_key] = _parse_int(base.get(int_key), 0 if int_key != "amount" else 100)
    for float_key in ["ai_score", "base_ai_score", "base_raw_score", "final_ai_score", "wind_speed", "wave_height", "water_state_score"]:
        base[float_key] = _parse_float(base.get(float_key), 0.0)

    if purchases is None:
        purchases = local_store.load_purchases(base["race_date"])
    purchase = dict(purchases.get(key, {}) or {})
    selected = str(purchase.get("selected") or purchase.get("purchased_selection_text") or base.get("purchased_selection_text") or "").strip()
    amount = normalize_amount_per_point(purchase.get("amount") or base.get("amount") or 100, 100)
    memo = str(purchase.get("memo") if purchase.get("memo") is not None else base.get("memo") or "")
    base["purchased_selection_text"] = selected
    base["amount"] = amount
    base["memo"] = memo
    base["purchased"] = 1 if selected else 0
    return base


def _rows_raw_by_date(date=None):
    return list(local_store.read_rows(date or today_text()))


def get_races_by_date(race_date):
    rows = _rows_raw_by_date(race_date)
    purchases = local_store.load_purchases(race_date)
    rendered = [_normalize_row_for_render(r, purchases) for r in rows if str(r.get("venue") or "").strip() != "テスト会場"]
    rendered.sort(key=lambda r: (str(r.get("time") or "99:99"), str(r.get("venue") or ""), int(r.get("race_no_num") or 0), str(r.get("candidate_source") or ""), int(r.get("id") or 0)))
    return rendered


def get_race_by_id(race_id):
    rid = _parse_int(race_id, 0)
    # 今日を先に探し、見つからなければ履歴も探す。
    dates = [today_text()] + [d for d in get_history_dates() if d != today_text()]
    for d in dates:
        for r in get_races_by_date(d):
            if int(r.get("id") or 0) == rid:
                return r
    return None


def _selected_items_for_row(row):
    return selection_items(row.get("purchased_selection_text", ""))


def _row_auto_values(row):
    selected = _selected_items_for_row(row)
    amount = normalize_amount_per_point(row.get("amount"), 100)
    result = normalize_pick_text(row.get("result_trifecta_text", ""))
    base_payout = _parse_int(row.get("result_trifecta_payout"), 0)
    hit = 1 if result and result in selected else 0
    payout = scale_payout_by_amount(base_payout, amount) if hit else 0
    investment = len(selected) * amount
    profit = payout - investment if selected else 0
    return selected, investment, hit, payout, profit
...
def update_race_result(race_id, selected_text, hit, payout, memo, amount_per_point=100):
    ...
    if not race:
        return
    date = str(race.get("race_date") or today_text())
    key = str(race.get("_local_key") or _local_row_key(race))
    purchases = local_store.load_purchases(date)
    selected_text = " / ".join(unique_preserve([normalize_pick_text(x) for x in selection_items(selected_text)]))
    if selected_text:
        purchases[key] = {
            "selected": selected_text,
            "amount": normalize_amount_per_point(amount_per_point, 100),
            "memo": str(memo or ""),
            "saved_at": jst_now_str(),
        }
    else:
        purchases.pop(key, None)
    local_store.save_purchases(purchases, date)
...
def delete_race(race_id):
    ...
    date = str(race.get("race_date") or today_text())
    key = str(race.get("_local_key") or _local_row_key(race))
    rows = [r for r in _rows_raw_by_date(date) if _local_row_key(r) != key]
    local_store.write_rows(rows, date)
    purchases = local_store.load_purchases(date)
    purchases.pop(key, None)
    local_store.save_purchases(purchases, date)
    return 1
...
def get_filtered_today_races(show_closed=False, ai_rating_filter="", official_rating_filter="pickup", base_quality_filter="", show_shadow=False, show_all_race=False, buy_rule_only=False):
    ...
    filtered = []
    for r in rows:
        source = normalize_candidate_source(r.get("candidate_source"))
        ...
        if official_rating_filter == "pickup":
            if rating not in {"★★★★★", "★★★★☆"}:
                continue
        elif official_rating_filter:
            if rating != official_rating_filter:
                continue
...
def get_summary_by_date(race_date):
    rows = [r for r in get_races_by_date(race_date) if normalize_candidate_source(r.get("candidate_source")) != "all_race_ai"]
    total_bets = total_points = total_investment = total_payout = total_hits = 0
    last_imported = ""
    for r in rows:
        last_imported = max(last_imported, str(r.get("imported_at") or ""))
        selected, investment, hit, payout, _profit = _row_auto_values(r)
        if selected:
            total_bets += 1
...
def get_group_summary(race_date, group_key):
    ...
    for r in rows:
        if group_key == "rating":
            name = str(r.get("rating") or "(空白)").strip() or "(空白)"
        elif group_key == "venue":
            name = str(r.get("venue") or "(空白)").strip() or "(空白)"
        elif group_key == "ai_rating":
            name = effective_ai_rating_text(r) or "(空白)"
        elif group_key == "final_rank":
            name = str(r.get("final_rank") or "(空白)").strip() or "(空白)"
        elif group_key == "candidate_source":
            name = candidate_source_label(r.get("candidate_source"))
        else:
            name = "(空白)"
        g = groups.setdefault(name, {"group_name": name, "total_bets": 0, "total_hits": 0, "total_points": 0, "total_investment": 0, "total_payout": 0})
        selected, investment, hit, payout, _profit = _row_auto_values(r)
        if selected:
            g["total_bets"] += 1
            g["total_points"] += len(selected)
            g["total_investment"] += investment
            g["total_payout"] += payout
            g["total_hits"] += hit
    results = []
    for g in groups.values():
        profit = g["total_payout"] - g["total_investment"]
        bets = g["total_bets"]
        inv = g["total_investment"]
        g.update({"total_profit": profit, "hit_rate": round(g["total_hits"] / bets * 100, 1) if bets else 0, "roi": round(g["total_payout"] / inv * 100, 1) if inv else 0})
        results.append(g)
    return sorted(results, key=lambda x: str(x["group_name"]))


def get_history_dates():
    dates = set()
    for p in local_store.DATA_DIR.glob("races_*.json"):
        name = p.stem
        if name == "races_latest":
            continue
        d = name.replace("races_", "", 1)
...
def upsert_base_candidates(cleaned):
    if not cleaned:
        return {"inserted": 0, "updated": 0}
    date = str(cleaned[0].get("race_date") or today_text())
    before = len(local_store.read_rows(date))
    rows = local_store.merge_records(cleaned, mode="base", date=date)
    after = len(rows)
    return {"inserted": max(0, after - before), "updated": len(cleaned) - max(0, after - before)}
...
def upsert_latest_candidates(cleaned):
    if not cleaned:
        return {"updated": 0, "skipped": 0}
    date = str(cleaned[0].get("race_date") or today_text())
    rows = local_store.merge_records(cleaned, mode="latest", date=date, freeze_closed=True)
    return {"updated": len(cleaned), "skipped": 0}


def _local_base_map_for_date(race_date):
    base_map = {}
    for r in get_races_by_date(race_date):
        venue = str(r.get("venue") or "").strip()
        race_no = str(r.get("race_no") or "").strip()
        source = normalize_candidate_source(r.get("candidate_source"))
        ...
        item = {
            ...
            "rating": str(r.get("rating") or "").strip(),
            "selection": str(r.get("selection") or "").strip(),
            "official_selection": str(r.get("selection") or "").strip(),
            "player_names_text": str(r.get("player_names_text") or "").strip(),
            "class_history_text": str(r.get("class_history_text") or "").strip(),
            "player_stat_text": str(r.get("player_stat_text") or "").strip(),
            "player_reason_text": str(r.get("player_reason_text") or "").strip(),
            "series_day": _parse_int(r.get("series_day"), 0),
            "race_phase": str(r.get("race_phase") or "").strip(),
            "base_ai_score": safe_float(r.get("base_ai_score"), 0),
            "base_ai_rating": str(r.get("base_ai_rating") or "").strip(),
            "base_ai_selection": str(r.get("base_ai_selection") or "").strip(),
            "base_reason_text": str(r.get("base_reason_text") or "").strip(),
            "base_raw_score": safe_float(r.get("base_raw_score"), 0),
            "base_raw_label": str(r.get("base_raw_label") or "").strip(),
            "base_raw_reason_text": str(r.get("base_raw_reason_text") or "").strip(),
            "final_ai_score": safe_float(r.get("final_ai_score"), 0),
        ...
        base_map[f"{venue}|{race_no}|{source}"] = item
        if source in {"official_star", "official_all"}:
            base_map[f"{venue}|{race_no}"] = item
    return base_map


def redirect_with_message(path, message_type, message_text):
    path = safe_redirect_path(path or "/", "/")
    sep = "&" if "?" in path else "?"
    return redirect(path + sep + "type=" + quote(message_type) + "&msg=" + quote(message_text))


def run_local_script(script_name, env_defaults=None, timeout_sec=480):
    ...
    try:
        ...
        return False, f"{script_name} がタイムアウトしました。PowerShellで直接実行して確認してください"
    except Exception as e:
        return False, f"{script_name} 実行エラー: {type(e).__name__}: {e}"

    output = ((completed.stdout or "") + "\n" + (completed.stderr or "")).strip()
    last_line = output.splitlines()[-1] if output else "出力なし"
    if completed.returncode != 0:
        return False, f"{script_name} 失敗: {last_line}"
    return True, f"直前更新しました: {last_line}"


@app.route("/run_latest_local", methods=["POST"])
def run_latest_local_route():
    ok, msg = run_local_script(
        "run_latest_local.py",
        env_defaults={
            "SKIP_PAST_RACES": "1",
            "RESULT_REPAIR_MODE": "0",
            "ENABLE_ALL_RACE_LIVE": "0",
            "LOCAL_FREEZE_CLOSED": "1",
        },
    )
    return redirect_with_message("/", "success" if ok else "error", msg)
...
def api_base_map_today():
    ...
    base_map = _local_base_map_for_date(race_date)
