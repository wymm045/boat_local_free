...
def make_player_stat_text(reason_map):
    """
    app側で数値の代わりに材料タグを出しやすくするための軽量版。
    まずは player_reason_text と同じタグ群を渡し、UI側で重複除去して表示する。
    """
    return make_player_reason_text(reason_map)
...
def make_base_candidate(row, beforeinfo_cache, racelist_cache, candidate_source="official_star"):
    ...
    candidate = {
        ...
        "player_stat_text": make_player_stat_text(player_reason_map),
