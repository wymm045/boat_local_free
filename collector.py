...
def extract_exhibition_times_from_context(lines):
    """
    beforeinfoのDOM/行単位で展示タイムが拾えない時の保険。
    ページ全体から6.20〜8.50の値を拾うが、明らかに6艇分の展示タイムらしい並びだけ採用する。
    """
    text = " ".join(str(x or "") for x in (lines or []))
    vals = []
    for raw in re.findall(r"(?<!\d)([6-8]\.\d{2})(?!\d)", text):
        try:
            v = float(raw)
        except Exception:
            continue
    ...
    if len(vals) >= 6:
        picked = vals[:6]
        if max(picked) - min(picked) <= 0.80:
            return [f"{v:.2f}" for v in picked]
    return []
...
def parse_beforeinfo_for_key(jcd, race_no):
    ...
    try:
        ...
        if len(exhibition_times) != 6:
            exhibition_times = extract_exhibition_times_from_context(lines)
        exhibition_ranks = build_exhibition_ranks_from_times(exhibition_times) if exhibition_times else {}

        if len(exhibition_times) == 6:
            log(f"[beforeinfo_exhibition_ok] jcd={jcd} race_no={race_no} times={','.join(exhibition_times)}")
        else:
            log(f"[beforeinfo_exhibition_missing] jcd={jcd} race_no={race_no} times_count={len(exhibition_times)}")
        ...
        log(f"[beforeinfo_parse_error] jcd={jcd} race_no={race_no} err={e}")
        return (jcd, race_no), empty_info
...
def calculate_latest_signal_metrics(exhibition_info, foot_material=None):
    ...
    if entry_change:
        signal_strength += min(0.32, entry_severity * 0.65)
        if course_map.get(1, 1) > 1:
            signal_strength += 0.06

    signal_strength = round(clamp(signal_strength, 0.0, 0.98), 2)
    latest_push = round(clamp(0.28 + signal_strength * 0.62, 0.26, 0.78), 2)
...
def stabilize_final_ai_score(base_ai_score, raw_final_ai_score, base_hold_strength, scenario_factor, signal_metrics=None, base_info=None):
    ...
    reflect_factor = clamp(reflect_factor, 0.26, 0.78)

    up_cap = 0.42 + signal_strength * 0.68 - float(base_hold_strength or 0) * 0.12 + max(0.0, float(scenario_factor or 1.0) - 0.78) * 0.08
    ...
    if base_quality_label == "base土台◎":
        reflect_factor = clamp(reflect_factor - 0.05, 0.24, 0.76)
        down_cap_mag = max(0.32, down_cap_mag - 0.14)
    elif base_quality_label == "base土台○":
        reflect_factor = clamp(reflect_factor - 0.03, 0.24, 0.77)
        down_cap_mag = max(0.38, down_cap_mag - 0.08)
    elif base_quality_label == "base保留":
        reflect_factor = clamp(reflect_factor - 0.02, 0.24, 0.76)
...
def build_role_score_maps(venue, exhibition_info, weather_info=None, foot_material=None, day_trend_bias=None, base_info=None, race_no=0):
    ...
    if len(st_map) >= 4:
        ...
        if len(sorted_st) >= 2:
            ...
            if best_st <= 0.10:
                head_score[best_lane] += 0.11
                second_score[best_lane] += 0.05
            elif best_st <= 0.12:
                head_score[best_lane] += 0.07
                second_score[best_lane] += 0.04
